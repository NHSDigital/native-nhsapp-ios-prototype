...
// MARK: Model (V1)
struct MessageV1: Identifiable, Hashable {
    let id = UUID()
    var sender: String
    var preview: String
    var date: Date
...
// MARK: Sample Message Data (V1)
let sampleMessagesV1: [MessageV1] = [
...
// MARK: - Date formatting helpers (V1)
fileprivate extension DateFormatter {
    static let messageTimeV1: DateFormatter = {
        let f = DateFormatter()
        f.dateFormat = "h:mma"
        f.amSymbol = "am"
        f.pmSymbol = "pm"
    ...
    static let messageWeekdayV1: DateFormatter = {
        let f = DateFormatter()
        f.dateFormat = "EEEE"
        f.locale = Locale(identifier: "en_GB")
        return f
    ...
    static let messageFullV1: DateFormatter = {
        let f = DateFormatter()
...
fileprivate func messageListDateStringV1(for date: Date, calendar: Calendar = .current) -> String {
    let now = Date()

    if calendar.isDateInToday(date) {
        return DateFormatter.messageTimeV1.string(from: date)
    }
    ...
    if weekNow == weekDate && yearNow == yearDate {
        return DateFormatter.messageWeekdayV1.string(from: date)
    }

    return DateFormatter.messageFullV1.string(from: date)
}

// MARK: - Full date format for detail view (V1)
fileprivate extension DateFormatter {
    static let messageFullLongV1: DateFormatter = {
        let f = DateFormatter()
...
fileprivate func messageDetailDateStringV1(for date: Date, calendar: Calendar = .current) -> String {
    let time = DateFormatter.messageTimeV1.string(from: date)
    ...
    if weekNow == weekDate && yearNow == yearDate {
        let weekday = DateFormatter.messageWeekdayV1.string(from: date)
        return "Received on \(weekday) at \(time)"
    }

    let full = DateFormatter.messageFullLongV1.string(from: date)
    return "Received \(full) at \(time)"
}

// MARK: Message row (V1)
private struct MessageRowV1: View {
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(alignment: .center) {
                    ...
                    HStack(spacing: 4) {
                        Text(messageListDateStringV1(for: message.date))
                            .font(.subheadline)
...
// MARK: - Message Detail View (V1)
private struct MessageDetailViewV1: View {
    let message: MessageV1
    @EnvironmentObject var messageStore: MessageStoreV1
    @Environment(\.dismiss) var dismiss
    @State private var showingRemoveAlert = false
    
    // Computed property to get current message state
    private var currentMessage: MessageV1 {
    ...
    var body: some View {
        ...
        .navigationSubtitle(messageDetailDateStringV1(for: message.date))
        .toolbar {
            ToolbarItem {
                Button {
...
// MARK: - Removed Messages View (V1)
private struct RemovedMessagesViewV1: View {
    @EnvironmentObject var messageStore: MessageStoreV1
    @State private var selectedMessage: MessageV1?
        ...
        ZStack {
            ...
            if messageStore.removedMessages.isEmpty {
                ...
                .navigationDestination(item: $selectedMessage) { message in
                    RemovedMessageDetailViewV1(message: message)
                }
            }
        }
...
// MARK: - Removed Message Detail View (V1)
private struct RemovedMessageDetailViewV1: View {
    let message: MessageV1
    @EnvironmentObject var messageStore: MessageStoreV1
    @Environment(\.dismiss) var dismiss
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    Text(messageDetailDateStringV1(for: message.date))
                        .font(.subheadline)
                        .foregroundStyle(.textSecondary)
                        .padding(.bottom, 16)
                        .padding(.top, -16)
...
// MARK: - Message Store (V1)
class MessageStoreV1: ObservableObject {
    @Published var messages: [MessageV1] = sampleMessagesV1
    @Published var removedMessages: [MessageV1] = []
    
    var unreadCount: Int {
        messages.filter { !$0.isRead }.count
    }
    
    func removeMessage(_ message: MessageV1) {
    ...
    func restoreMessage(_ message: MessageV1) {
    ...
    func permanentlyDelete(_ message: MessageV1) {
        removedMessages.removeAll { $0.id == message.id }
    }
}

// MARK: List + Navigation + Swipe (V1)
struct MessagesViewV1: View {
    ...
    // Base filtered list (no search)
    private var filteredMessages: [MessageV1] {
        let baseList: [MessageV1]
        switch filter {
        case .all:
    ...
    var body: some View {
        NavigationStack {
            if messageStore.messages.isEmpty {
                ...
                .navigationSubtitle(Text(unreadSubtitle).font(.title2))
                .navigationBarTitleDisplayMode(.large)
                .toolbar { filterToolbar }
            } else {
                List {
                    if filteredMessages.isEmpty {
                        Text("No \(filter.rawValue.lowercased()) messages")
                            .foregroundStyle(.textSecondary)
                ...
                .background(Color.pageBackground)
                .navigationDestination(item: $selectedMessage) { message in
                    MessageDetailViewV1(message: message)
                }
                .toolbar { filterToolbar }
